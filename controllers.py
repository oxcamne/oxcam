...
# .../member_reservations/member_id/...
	...
	header = CAT(A('back', _href=session['url_prev']),
				...
				A('Add New Reservation', _href=URL(f'add_member_reservation/{member_id}',
					vars=dict(back=session['url']), scheme=True)))
	...
	grid = Grid(path, (db.Reservations.Member==member_id)&(db.Reservations.Host==True),
			...
			columns=[db.Events.DateTime,
	    			Column('event', lambda row: A(db.Events[db.Reservations[row.Reservations.id].Event].Description, _href=URL(f"reservation/N/{member_id}/{db.Reservations[row.Reservations.id].Event}/select"),
								   _style="white-space: normal")),
				    Column('wait', lambda row: res_wait(member_id, db.Reservations[row.Reservations.id].Event) or ''),
				    Column('conf', lambda row: res_conf(member_id, db.Reservations[row.Reservations.id].Event) or ''),
				    Column('cost', lambda row: event_cost(db.Reservations[row.Reservations.id].Event, member_id) or ''),
				    Column('tbc', lambda row: event_unpaid(db.Reservations[row.Reservations.id].Event, member_id) or '')],
			grid_class_style=grid_style,
			formstyle=form_style,
...
def add_member_reservation(member_id):
	access = session['access']	#for layout.html
	header = CAT(A('back', _href=request.query.get('back')),
...
def new_members(path=None):
	...
	grid = Grid(path, db.AccTrans.id.belongs(ids), orderby=~db.AccTrans.Timestamp,
			columns=[Column("Name", lambda row: A(member_name(db.AccTrans[row.id].Member), _href=URL(f"members/{'details' if access=='read' else 'edit'}/{db.AccTrans[row.id].Member}"),
								   _style="white-space: normal")),
	    			Column("College", lambda row: primary_affiliation(db.AccTrans[row.id].Member)),
	    			Column("Matr", lambda row: primary_matriculation(db.AccTrans[row.id].Member)),
					Column("Status", lambda row: db.Members[db.AccTrans[row.id].Member].Membership),
					Column('Date', lambda row: db.AccTrans[row.id].Timestamp.strftime(DATE_FORMAT)),
					Column('Previous', lambda row: classify(db.AccTrans[row.id])),
					Column("Source", lambda row: db.Members[db.AccTrans[row.id].Member].Source)],
...
def events(path=None):
	...
	grid = Grid(path, db.Events.id>0,
			...
			columns=[db.Events.DateTime,
					Column('event', lambda row: A(db.Events[row.id].Description, _href=URL(f"event_reservations/{row.id}"),
								   _style="white-space: normal")),
	   				db.Events.Venue,
...
select(db.Reservations.Member, orderby=db.Reservations.Member, distinct=True)])"
	...
	grid = Grid(path, eval(query),
			...
			columns=[Column('member', lambda row: A(member_name(db.Reservations[row.Reservations.id].Member), _href=URL(f"reservation/N/{db.Reservations[row.Reservations.id].Member}/{event_id}/select"),
										   _style='white-space: normal')),
	    				db.Members.Membership, db.Members.Paiddate, db.Reservations.Affiliation, db.Reservations.Notes,
					    Column('cost', lambda row: event_cost(event_id, db.Reservations[row.Reservations.id].Member) or ''),
					    Column('tbc', lambda row: event_unpaid(event_id, db.Reservations[row.Reservations.id].Member) or ''),
					    Column('count', lambda row: (res_wait(db.Reservations[row.Reservations.id].Member, event_id) if request.query.get('waitlist')\
				      		else res_prov(db.Reservations[row.Reservations.id].Member, event_id) if request.query.get('provisional')\
							else res_conf(db.Reservations[row.Reservations.id].Member, event_id)) or'')],
...
# in the latter case, ismember=='Y'
	...
	event_tickets = [(t.id, f"{t.Ticket}{' (waitlisting)' if t.Waiting else ''}") for t in tickets if \
				ismember!='Y' or\
	...
	for t in tickets:
		if t.Count:
			tickets_available[t.Ticket] = t.Count - tickets_sold(t.id) if not t.Waiting else 0
	...
	for row in all_guests:
		...
		if not row.Waitlist:
			if row.Provisional:
				...
				if ticket.Ticket in tickets_available:
					tickets_available[ticket.Ticket] -= 1
					if tickets_available[ticket.Ticket] == 0:
						flash.set(f"There are no further {ticket.Ticket} tickets for additional guests.")
					elif tickets_available[ticket.Ticket] < 0:
				...
				confirmed += 1
				confirmed_ticket_cost += ticket.Price
...
Moving member on/off waitlist will also affect all guests."))
	...
	if path and path != 'select' and not path.startswith('delete'):	#editing or creating reservation
		if ismember=='Y':
			...
			db.Reservations.Survey_.readable = True
		...
		if host_reservation and (path=='new' or host_reservation.id!=int(path[path.find('/')+1:])):
			...
			if ismember!='Y':
				...
				for t in event_tickets:
					if is_good_standing:
						if t[1].lower().startswith(membership.lower()):
							db.Reservations.Ticket_.default = t[0]
							db.Reservations.Ticket_.writable = False
					elif t[1].lower().startswith('non-member'):
						db.Reservations.Ticket_.default = t[0]
						db.Reservations.Ticket_.writable = False
			...
			if ismember=='Y' and path=='new' and not selections and \
				(len(event_tickets)<=1 or db.Reservations.Ticket_.writable==False):
				...
				db.Reservations.insert(Member=member_id, Event=event_id, Host=True,
					...
					Ticket_=db.Reservations.Ticket_.default)
	...
	def validate(form):
		...
		if ismember=='Y' and db.Reservations.Ticket_.writable==True and \
				form.vars.get('Ticket_')!='' and int(form.vars.get('Ticket_')) != db.Reservations.Ticket_.default:
			ticket = db.Event_Tickets[form.vars.get('Ticket_')]
		...
		if len(form2.errors)>0:
			...
			for row in all_guests:
				if row.Provisional==True:
					if row.Ticket_:
						ticket = db.Event_Tickets[row.Ticket_]
						if ticket.Ticket in tickets_available and tickets_available[ticket.Ticket] <= 0:
	...
	try:
		...
		for host in hosts:
			...
			for guest in guests:
				...
				survey = db.Event_Survey[guest.Survey_].Item if guest.Survey_ else ''
...
def financial_detail(event, title=''):
	...
	if not request.query.get('link_back'):
		session['back'].append(session['url_prev'])
...
def transactions(path=None):
	...
	grid = Grid(path, eval(request.query.get('query') or 'db.AccTrans.id>0'), left=eval(request.query.get('left')) if request.query.get('left') else None,
			...
			columns=[db.AccTrans.Timestamp, db.AccTrans.Account, db.AccTrans.Event,
					Column('member', lambda row: A(member_name(db.AccTrans[row.id].Member), _href=URL(f"members/edit/{db.AccTrans[row.id].Member}"),
										   _style='white-space: normal') if db.AccTrans[row.id].Member else ''),
