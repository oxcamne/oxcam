...
db.define_table('Emails',
	...
	Field('Mailings', 'list:reference Email_Lists', #widget=ListRefCheckboxWidget,
			requires=IS_EMPTY_OR(IS_IN_DB(db(db.Email_Lists.id>0), db.Email_Lists.id, '%(Listname)s', multiple=True)),
...
db.define_table('Events',
	...
	Field('EndTime', 'datetime', requires=IS_EMPTY_OR(IS_DATETIME()), comment=" (optional) Event End Date and Time"),
	...
	Field('Members_only', 'boolean', default=MEMBERSHIPS!=[], comment=" members and their guests only",
	   		readable=MEMBERSHIPS!=[], writable=MEMBERSHIPS!=[]),
	Field('Hidden', 'boolean', default=False, comment=" not shown on event calendar"),
...
db.define_table('Event_Tickets',
	...
	Field('Short_name', 'string', comment="short name should match membership category if New_member is set, used for doorlist"),
...
db.define_table('Pages',
	...
	Field('Hide', 'boolean', default=False, comment="hide from menu"),
	Field.Virtual('Parent_name', lambda r: page_name(r.Parent)),
	Field('Link', 'string', comment="use external Link (leave Content empty)"),
	Field('Content', 'text', 
	   comment=CAT(markdown_comment, ", [[function_name(parameters)]] for dynamic content")),
	Field('Views', 'integer', default=0, writable=False),
	Field('Created', 'datetime', default=lambda: datetime.datetime.now(TIME_ZONE).replace(tzinfo=None), writable=False),
	Field('Modified', 'datetime', default=lambda: datetime.datetime.now(TIME_ZONE).replace(tzinfo=None),
       			update=lambda: datetime.datetime.now(TIME_ZONE).replace(tzinfo=None), writable=False),
...
db.define_table('Bank_Accounts',
	...
	Field('Debit', 'string', comment=" Column name containing debit amount (if separate from credit)"),
