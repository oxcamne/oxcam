			...
			for i in dispatch:
				...
				for part in bodyparts:
					if part[0]:
						...
						body += f"<br><br><a href={notice.Scheme}unsubscribe/{row.get(db.Emails.id)}/{mailing_list.id}/{member.id}/{row.get(db.Emails.Email)}>Unsubscribe</a> from '{mailing_list.Listname}' mailing list."
				retry_seconds = 2
				while True:
					try:
